...
export const NON_SCALABLE_REGEX = new RegExp(`(?:${nonScalableKeywords.map(escapeRegExp).join("|")})`);
// 数値表現（分数・範囲を含む）
export const NUMBER_PATTERN = /(\d+(?:\.\d+)?(?:[\/／]\d+)?(?:\s*[〜～~-]\s*\d+(?:\.\d+)?(?:[\/／]\d+)?)?)/g;
// 単位抽出の高速化: 長いキー順
export const SORTED_UNIT_ENTRIES = Object.entries(unitMap)
...
export function stripAndCollectParentheticals(text) {
    const parens = [];
    const parensInner = [];
    let work = text;
...
export function checkNonScalableKeywords(text) {
